...
| Abuse report for a specific note | Reported note ID needs removal | `nyanbin-admin revoke <id> --yes`; optionally `nyanbin-admin block <bucket>` if a rejection pattern identifies the creating bucket. Never attempt to decrypt or inspect content — the operator cannot, and that is the point |
