...
<style>.drop { display: grid; min-height: 7rem; place-items: center; padding: var(--space-4); border: 2px dashed var(--border-strong); border-radius: var(--radius-md); background: var(--surface-blue); text-align: center; transition: background var(--duration-ui) var(--ease-out), border-color var(--duration-ui) var(--ease-out); } .drop:hover:not([aria-disabled='true']), .drop.dragging { border-color: var(--blue-600); background: var(--surface-strong); } label { width: 100%; cursor: pointer; } input { margin-block: var(--space-2); padding: var(--space-2); background: var(--surface); } small { display: block; color: var(--ink-muted); }</style>
