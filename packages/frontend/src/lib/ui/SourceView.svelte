...
<style>pre { margin: 0; padding: var(--space-4); overflow: auto; border: 1px solid var(--border); border-radius: var(--radius-sm); background: var(--surface-blue); font-size: var(--text-sm); line-height: 1.6; tab-size: 2; } .line { display: grid; grid-template-columns: 3ch minmax(0, 1fr); min-height: 1.6em; } .number { color: var(--ink-muted); user-select: none; } .keyword { color: var(--blue-700); font-weight: 700; } .string { color: var(--success); } .comment { color: var(--ink-muted); font-style: italic; } .number:not(:first-child) { color: var(--danger); }</style>
