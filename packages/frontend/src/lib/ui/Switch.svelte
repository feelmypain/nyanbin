...
<style>
	...
	.track { position: relative; display: block; flex: 0 0 auto; width: 3.25rem; height: 1.75rem; border: 1px solid var(--border-strong); border-radius: 99rem; background: var(--surface-accent); transition: background var(--duration-ui) var(--ease-out), border-color var(--duration-ui) var(--ease-out); }
	...
	label:hover .track { border-color: var(--accent-600); }
	input:checked + .track { border-color: var(--accent-700); background: var(--accent-600); }
