<script lang="ts">
	...
	let boxColor = $derived($theme === 'red' || $theme === 'green' || $theme === 'pink' ? $theme : 'blue')
	...
	let expiryChoices = $derived.by(() => {
		const candidates = [60, 300, 1_800, 3_600, 21_600, 86_400, 604_800, expiresIn]
...
<section class="intro"><div><span class="eyebrow">{$t('create.eyebrow')}</span><h1>{$t('create.title', { values: { color: $t(`create.box_color.${boxColor}`) } })}</h1><p>{$t('create.intro')}</p></div><div class="cat-art" aria-hidden="true"><span class="tail"></span><span class="cat-head">⌁<b>•ᴗ•</b>⌁</span></div></section>
...
<form class="panel composer" data-testid="create-form" onsubmit={submit} onpaste={addClipboardFiles}>
	<fieldset disabled={busy || $status.state !== 'ready'}>
		<div class="editor">
			...
			{#if filesEnabled}<div class="attach"><FileUpload bind:files disabled={busy || $status.state !== 'ready'}/><PastedFilesPreview bind:files/></div>{/if}
...
{/if}
<style>
	.intro { display: grid; grid-template-columns: minmax(0, 1fr) auto; align-items: end; gap: var(--space-5); margin-bottom: var(--space-5); } .intro h1 { max-width: 18ch; margin-top: var(--space-3); font-size: var(--text-2xl); background: linear-gradient(92deg, var(--accent-700) 20%, var(--accent-500)); -webkit-background-clip: text; background-clip: text; color: transparent; } .intro p { max-width: 60ch; margin-top: var(--space-2); color: var(--ink-soft); font-size: var(--text-lg); } .cat-art { position: relative; width: 9rem; height: 4.5rem; color: var(--accent-600); } .cat-head { position: absolute; inset-block-end: 0; inset-inline-end: 0; padding: var(--space-3); border: 2px solid var(--accent-600); border-radius: 45% 45% 35% 35%; background: var(--surface-accent); font: var(--text-sm)/1 var(--font-mono); animation: bob 8s ease-in-out infinite; } .cat-head::before, .cat-head::after { content: ''; position: absolute; inset-block-start: -.58rem; width: 0; height: 0; border-inline: .42rem solid transparent; border-block-end: .68rem solid var(--accent-600); } .cat-head::before { inset-inline-start: .7rem; transform: rotate(-14deg); } .cat-head::after { inset-inline-end: .7rem; transform: rotate(14deg); } .cat-head b { margin-inline: var(--space-1); } .tail { position: absolute; width: 5rem; height: 3rem; inset-block-end: .25rem; inset-inline-start: .5rem; border-block-end: .4rem solid var(--accent-600); border-inline-start: .4rem solid var(--accent-600); border-radius: 0 0 0 100%; transform-origin: 100% 100%; animation: tail 6.5s ease-in-out infinite; } @keyframes bob { 50% { transform: translateY(-.2rem); } } @keyframes tail { 50% { transform: rotate(3deg); } }
	.status-error { display: flex; align-items: center; justify-content: space-between; gap: var(--space-4); margin-bottom: var(--space-4); }.composer > fieldset { display: grid; grid-template-columns: minmax(0, 2fr) 18rem; } .editor { min-width: 0; padding: var(--space-5); } .attach { margin-top: var(--space-5); } .options { display: flex; min-width: 0; flex-direction: column; gap: var(--space-4); padding: var(--space-5); border-inline-start: 1px solid var(--border); background: var(--surface-accent); border-radius: 0 var(--radius-lg) var(--radius-lg) 0; } .format-row { display: flex; align-items: end; justify-content: space-between; gap: var(--space-3); margin-bottom: var(--space-3); } .formats { display: flex; flex-wrap: wrap; gap: var(--space-2); } .formats legend { width: 100%; margin-bottom: var(--space-1); } .formats label { display: inline-flex; align-items: center; gap: var(--space-2); min-height: 2.25rem; padding: var(--space-1) var(--space-3); border: 1px solid var(--border-strong); border-radius: 99rem; background: var(--surface); cursor: pointer; font-weight: 700; transition: border-color var(--duration-ui) var(--ease-out), background var(--duration-ui) var(--ease-out); } .formats label:hover { border-color: var(--accent-600); } .formats label:has(input:checked) { border-color: var(--accent-600); background: var(--surface-strong); } .formats input { width: 1.05rem; min-height: 1.05rem; margin: 0; accent-color: var(--accent-600); } .preview-toggle { min-width: 6rem; } .preview { min-height: 18rem; margin-bottom: var(--space-4); padding: var(--space-4); border: 1px solid var(--border-strong); border-radius: var(--radius-sm); overflow: auto; } .preview pre { white-space: pre-wrap; overflow-wrap: anywhere; } .empty-preview { color: var(--ink-muted); } .local-note { margin-block: var(--space-4) 0; color: var(--ink-soft); } details { padding: var(--space-2) var(--space-4); border: 1px solid var(--border-strong); border-radius: var(--radius-md); background: var(--surface); transition: border-color var(--duration-ui) var(--ease-out); } details:hover, details[open] { border-color: var(--accent-600); } details summary { min-height: 2.75rem; cursor: pointer; font-weight: 700; } details p { margin-block: var(--space-3) var(--space-2); padding-top: var(--space-3); border-top: 1px dashed var(--border); color: var(--ink-muted); font-size: var(--text-sm); line-height: 1.6; } .summary { display: grid; gap: var(--space-1); padding-block: var(--space-3); border-block: 1px solid var(--border); color: var(--ink-muted); font-size: var(--text-sm); } .create { width: 100%; } .live { min-height: 1px; } .form-error { margin: var(--space-4); }
	.create .help { margin-top: var(--space-3); text-align: center; }
