...
<style>.center { display: flex; min-height: 18rem; align-items: center; justify-content: center; gap: var(--space-2); }.empty, .gate { max-width: 42rem; margin-inline: auto; padding: var(--space-5); }.empty { display: grid; gap: var(--space-4); text-align: center; }.sleeping { color: var(--blue-600); font: 700 var(--text-xl)/1 var(--font-mono); }.gate header { display: flex; align-items: center; justify-content: space-between; gap: var(--space-4); margin-bottom: var(--space-4); }.cat { color: var(--blue-600); font: 700 var(--text-xl)/1 var(--font-mono); }dl { display: grid; grid-template-columns: 1fr 1fr; margin-block: var(--space-4); border-block: 1px solid var(--border); }dl div { padding: var(--space-3); }dt { color: var(--ink-muted); font-size: var(--text-sm); }dd { margin: 0; font-weight: 700; }form { display: grid; gap: var(--space-4); }form :global(button) { width: 100%; }.gate > .help { margin-top: var(--space-4); }@media (max-width: 30rem) { .empty, .gate { padding: var(--space-4); } .cat { display: none; } dl { grid-template-columns: 1fr; } }</style>
