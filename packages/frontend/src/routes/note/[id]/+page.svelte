<script lang="ts">
	...
	async function loadInfo() {
		...
		try {
			...
			// A bare link can only decrypt a password-protected note; anything else is unusable.
			if (!secret && !info.passwordProtected) {
				revealPhase = 'invalid'
				return
			}
			revealPhase = 'gate'
	...
	onMount(async () => {
		...
		if (hash !== '') {
			try {
				secret = decodeBase64Url(hash, { length: SECRET_BYTES, label: 'link secret' })
			} catch {
				revealPhase = 'invalid'
				return
			}
	...
	async function reveal(event: SubmitEvent) {
		...
		if (!info) return
		if (!secret && password === '') {
			error = $t('show.errors.password_required')
			return
		}
		error = ''
		try {
			...
			payload = await decryptPayload(response.envelope, { id: data.id, ...(secret ? { secret } : {}), password, lifecycle: { expiresAt: info.lifecycle.expiresAt, ...(info.lifecycle.maxReads === undefined ? {} : { maxReads: info.lifecycle.maxReads }) } })
			revealPhase = 'shown'
...
{:else}<section class="gate panel" data-testid="reveal-gate" aria-labelledby="gate-title"><header><div><span class="eyebrow">{$t('show.eyebrow')}</span><h1 id="gate-title">{$t('show.gate_title')}</h1></div><span class="cat" aria-hidden="true">/ᐠ - ˕ -マ</span></header><p class="notice warning">{$t('show.consume_warning')}</p>{#if info}<dl data-testid="note-lifecycle"><div><dt>{$t('show.expires')}</dt><dd>{formatExpiry(info.lifecycle.expiresAt, $locale)}</dd></div>{#if info.lifecycle.remainingReads !== undefined}<div><dt>{$t('show.remaining')}</dt><dd>{info.lifecycle.remainingReads}</dd></div>{/if}</dl>{/if}{#if error}<p class="notice error" role="alert">{error}</p>{/if}<form onsubmit={reveal}><TextInput id="show-note-password" data-testid="show-note-password" type="password" reveal autocomplete="current-password" required={!secret} label={!secret ? $t('show.password_required_label') : $t('show.password_optional')} help={!secret ? $t('show.password_required_help') : $t('show.password_help')} bind:value={password}/><Button data-testid="show-note-button" variant="primary" type="submit" disabled={revealPhase === 'revealing' || revealPhase === 'decrypting'}>{#if revealPhase === 'revealing'}<Loader/> {$t('show.revealing')}{:else if revealPhase === 'decrypting'}<Loader/> {$t('show.decrypting')}{:else}{$t('show.reveal')}{/if}</Button></form><p class="help">{!secret ? $t('show.password_shared_help') : $t('show.fragment_help')}</p></section>{/if}
